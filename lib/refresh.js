 ...
 * used to supply a custom OAuth2 adapter, or modify the one
 ...
 *       oauth2: new OAuth2(...),
 *       modifyOAuth2({ strategyOAuth2, refreshOAuth2 }) {
 ...
 * @param {OAuth2} options.oauth2 a custom oauth2 adapter to use
 * @param {OAuth2} options.modifyOAuth2 a callback to modify the oauth2 adapter. Should return the oauth2 adapter to use when refreshing the token.
 */
AuthTokenRefresh.use = function (name, strategy, options) {
  ...
  // Start with the supplied oauth2 object
  let refreshOAuth2 = options.oauth2;

  if (!refreshOAuth2) {
    // Try to use the internal oauth2 adapter, which is set by some strategies
    if (!strategy._oauth2) {
  ...
  if (typeof options.modifyOAuth2 === 'function') {
    refreshOAuth2 = options.modifyOAuth2({
